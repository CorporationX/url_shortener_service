...
CREATE
OR REPLACE FUNCTION generate_base62(length integer)
RETURNS text AS $$
DECLARE
...
values ('sleueh', 'http://21it.ru', CURRENT_TIMESTAMP - INTERVAL '2 years');
insert into url (hash, url, created_at)
values ('7leueh', 'http://21it.ru', CURRENT_TIMESTAMP - INTERVAL '2 years');
insert into url (hash, url)
values ('sleif', 'http://21it.ru');