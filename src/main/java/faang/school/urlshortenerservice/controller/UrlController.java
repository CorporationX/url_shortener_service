...
public class UrlController {
    ...
    @GetMapping("/{hash}")
    public String  getLongUrl(@PathVariable
                                   @Size(min = 6, max = 6, message = "The hash must be 6 characters")
                                   String hash) {
        return urlService.getOriginalUrl(hash);
        //return new RedirectView(urlService.getOriginalUrl(hash)); //todo
    }
