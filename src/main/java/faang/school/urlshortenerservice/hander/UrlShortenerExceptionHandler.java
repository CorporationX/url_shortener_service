...
import faang.school.urlshortenerservice.dto.UrlErrorResponseDto;
...
public class UrlShortenerExceptionHandler {
    ...
    public ResponseEntity<UrlErrorResponseDto> handleServerException(Exception ex) {
    ...
    private ResponseEntity<UrlErrorResponseDto> createErrorResponse(String errorMsg,
                                                                    HttpStatus status,
                                                                    Exception ex) {
        log.error("Error in url-shortener-service: {}, response status {}", errorMsg, status, ex);
        UrlErrorResponseDto response =
                new UrlErrorResponseDto(errorMsg, LocalDateTime.now(), status.value());
