...
public class HashGenerator {
    ...
    @Value("${spring.hash.range}")
    private int maxRange;

    private final HashRepository hashRepository;
    private final Base62Encoder base62Encoder;

    @Scheduled(cron = "${spring.hash.cron}")
    @Transactional
    public void generateHash() {
        ...
        log.info("Hash pool generated. Count: {}", hashes.size());
    }
    ...
    public List<String> getHashes(int amount) {
        ...
        if (hashes.size() < amount) {
            log.warn("Not enough hashes in hash pool. Generating more...");
            generateHash();
            hashes.addAll(hashRepository
                    .getHashBatchAndDelete(amount - hashes.size()));
        }
    ...
    public CompletableFuture<List<String>> getHashesAsync(int amount) {
        log.info("Getting {} hashes asynchronously...", amount);
