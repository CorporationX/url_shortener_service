...
public class HashRepository {
    ...
    public List<Long> getUniqueNumbers(long uniqueNumbers) {
        String sql = "select nextval('unique_number_sequence') from unique_number_sequence(1, ?)";
    ...
    public List<Hash> getHashBatch(long uniqueNumbers) {
        String sql = "delete from hash where ctid IN (SELECT ctid FROM hash LIMIT ?) RETURNING *";
        return jdbcTemplate.queryForList(
                sql, Hash.class, uniqueNumbers);
    }
}
