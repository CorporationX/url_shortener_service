...
public class HashRepository {
    ...
    public List<String> getAndDeleteHashes(int amount) {
        return jdbcTemplate.query("delete from hash where id in ( select hash.id from hash order by random() limit ?) returning hash", (rs, rowNum) -> rs.getString("hash"), amount);
    }
}
