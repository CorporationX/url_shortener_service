...
public class UrlServiceImpl implements UrlService {
    ...
    @Value("${hash.url.base-shortener-url}")
    private String baseShortenerUrl;
    ...
    public String createShortUrl(UrlRequest urlRequest) {
        ...
        return URI.create(baseShortenerUrl).resolve(hash).toString();
    }
    ...
    public String getOriginalUrl(String hash) {
        ...
        return urlRepository.findById(hash)
                .map(Url::getUrl)
                .orElseThrow(() -> new NotFoundException(hash));
    }
}
