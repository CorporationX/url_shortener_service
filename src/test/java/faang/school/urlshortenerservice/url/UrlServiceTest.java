...
public class UrlServiceTest {
    ...
    public void createTest() {
        ...
        String result = service.create(testDto);
        assertNotNull(result);
        assertTrue(result.endsWith(hash));

        verify(redisCache, times(1)).save(hash, url);
        verify(urlRepository, times(1)).save(argThat(saved ->
                saved.getHash().equals(hash) && saved.getUrl().equals(url)
        ));
    }
    ...
    public void couldNotFindTest() {
        ...
        assertThrows(UrlNotFoundException.class, () -> service.find(hash));
    }
}
