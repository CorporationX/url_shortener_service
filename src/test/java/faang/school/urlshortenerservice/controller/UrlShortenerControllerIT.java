...
import faang.school.urlshortenerservice.service.HashPoolService;
...
class UrlShortenerControllerIT {
    ...
    void testCreateAndRedirectShortUrl() throws Exception {
        doNothing().when(hashPoolService).warmUpCache();
        when(hashPoolService.getAvailableHash()).thenReturn(new FreeHash("hash"));
        hashPoolService.warmUpCache();
        UrlRequest request = new UrlRequest("https://example.com", 5);

        String response = mockMvc.perform(post("/hash/get")
